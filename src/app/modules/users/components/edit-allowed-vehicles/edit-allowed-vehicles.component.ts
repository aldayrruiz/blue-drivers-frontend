...
import { ErrorMessageService, SnackerService, UserService } from '@core/services';
...
export class EditAllowedVehiclesComponent implements OnInit {
  ...
  checkboxLabel(row?: Vehicle): string {
    ...
    return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row ${this.vehicles.indexOf(row) + 1}`;
  }
